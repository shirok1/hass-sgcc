...
class ONNX:
    ...
    def draw(self, image, box_data):
        ...
        for box, score, cl in zip(boxes, scores, classes):
            ...
            draw.text(
                xy=(top, left),
                text="{0} {1:.2f}".format(CLASSES[cl], score),
                fill=(255, 0, 0),
            )
    ...
    def get_boxes(self, prediction, confidence_threshold=0.7, nms_threshold=0.6):
        ...
        for i in range(len(cls_cinf)):
            cls.append(
                int(np.argmax(cls_cinf[i]))
            )  # 剩下的objecctness score比较大的grid cell，分别对应的预测类别列表
        ...
        for i in range(len(all_cls)):
            ...
            curr_cls_box = np.array(
                curr_cls_box
            )  # 0 1 2 3 4 5 分别是 x y w h score class
            curr_cls_box = self.xywh2xyxy(
                curr_cls_box
            )  # 0 1 2 3 4 5 分别是 x1 y1 x2 y2 score class
            curr_out_box = self.nms(
                curr_cls_box, nms_threshold
    ...
    def letterbox(
        ...
        img = ImageOps.expand(
            img, border=(left, top, right, bottom), fill=0
