...
def main():
    ...
    parsed_time = datetime.strptime(JOB_START_TIME, "%H:%M") + timedelta(
        minutes=random_delay_minutes
    )
    logging.info(
        f"The current logged-in user name is {PHONE_NUMBER}, the homeassistant address is {HASS_URL}, and the program will be executed every day at {parsed_time.strftime('%H:%M')}."
    )
    ...
    logging.info(
        f"Run job now! The next run will be at {parsed_time.strftime('%H:%M')} and {next_run_time.strftime('%H:%M')} every day"
    )
